...
const TABS: { id: Tab; label: string; labelShort: string; icon: string }[] = [
  ...
  { id: "contado", label: "Métodos Contado",     labelShort: "Contado",       icon: "M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" },
  { id: "credito", label: "Métodos Crédito",     labelShort: "Crédito",       icon: "M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" },
];
...
const ItemRow = ({
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-800 last:border-0 hover:bg-slate-800/30 transition-colors">
      {showColor && (
      ...
      {editing ? (
        ...
        <span className="flex-1 text-sm text-slate-300 min-w-0 truncate">{name}</span>
      )}
      ...
      <div className="flex items-center gap-1 flex-shrink-0">
        {editing ? (
          ...
          <>
            {/* Investment toggle — always visible */}
            {showInvestmentToggle && (
              <button
                onClick={handleToggleInvestment}
                disabled={toggling}
                title={isInvestment ? "Quitar marca de inversión" : "Marcar como inversión"}
            ...
            <button onClick={() => { setEditing(true); setConfirming(false); }}
              className="p-1.5 text-slate-500 hover:text-indigo-400 hover:bg-indigo-500/10 rounded-lg transition-colors" title="Editar">
...
const Configuracion = () => {
  ...
  const handleAdd    = async (name: string) => {
    if (isCategory) await createCategory(name, tab as Category["type"]);
    else            await createPayChannel(name, tab as PayChannel["type"]);
  };
  ...
  const addPlaceholder = isCategory
    ...
    : `Nuevo método de ${tab === "contado" ? "contado" : "crédito"}...`;
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      <div className="bg-slate-900 rounded-xl border border-slate-800 shadow-lg overflow-hidden">
        ...
        {/* Tabs — 2×2 grid on mobile, horizontal on desktop */}
